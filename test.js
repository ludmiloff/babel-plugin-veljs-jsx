...
test(
  "Transforms virtual element class",
  '<div><MyComponent prop="test"/></div>;',
  'self.part(5)`<div>${MyComponent.for(self, "_f5_", {\n'
    + '  prop: "test"\n'
    + '})}</div>`;'
)
...
test(
  ...
  'self.part(6)`<input type="text" value=\'foo\'>`;'
)

test(
  "Transforms expression attributes",
  `<input value={val}/>;`,
  'self.part(7)`<input value=${val}>`;'
)

test(
  "Doesn't transform entities in string attributes",
  `<input value="&quot;"/>;`,
  'self.part(8)`<input value="&quot;">`;'
)

test(
  "Escapes string attributes",
  `<input value='\\\\' placeholder="\`"/>;`,
  `self.part(9)\`<input value='\\\\\\\\' placeholder="\\\`">\`;`
)

test(
  "Transforms boolean attributes (default true value)",
  `<input disabled/>;`,
  'self.part(10)`<input disabled>`;'
)

test(
  "Transforms boolean attributes (false)",
  `<input disabled="true"/>;`,
  'self.part(11)`<input disabled="true">`;'
)

test(
  "Transforms expression attributes",
  `<input value={val}/>;`,
  'self.part(12)`<input value=${val}>`;'
)
...
test(
  ...
  `<p> foo: <b> {val} </b> </p>;`,
  'self.part(23)`<p> foo: <b> ${val} </b> </p>`;'
)

test(
  "Transforms fragment children",
  `<p> foo: <>{val}</> </p>;`,
  'self.part(24)`<p> foo: ${val} </p>`;'
)
...
test(
  ...
  'self.part(25)`<div class="some-class" onclick=${console.log}></div>`;'
)
